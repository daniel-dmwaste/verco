...
| **P0-10** | **`client` table has SELECT policies but no INSERT/UPDATE/DELETE policies.** Admin branding form silently no-ops because `updateClient` calls `.update()` without `.select()`, so an RLS deny returns `{error:null,data:null}` and the action returns `ok:true`. User reported branding edits "reverting on refresh" — confirmed root cause. Same gap silently breaks `createNewClient`. | `supabase/migrations/20260326160000_public_read_policies.sql:14` (only SELECT exists); `src/app/(admin)/admin/clients/actions.ts:133-137` (no `.select()` chained); reported by Dan 2026-05-13 | Admins cannot configure tenant branding. White-labelling — the entire P1 of UI_UX_AUDIT — is unreachable from the UI. Tester #1 will hit this immediately. | **Fixed in #32** — new migration adds INSERT/UPDATE/DELETE for contractor-admin (own contractor's clients) + UPDATE for client-admin (own client only); actions chain `.select('id').single()` so future RLS gaps fail loud. ~30 min total. |

**Total P0 effort: ~3 hours (P0-10 already addressed in #32).**
